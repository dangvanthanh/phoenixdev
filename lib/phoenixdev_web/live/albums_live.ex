defmodule PhoenixdevWeb.AlbumsLive do
    ...
    <div class="flex h-screen bg-slate-200">
      <div class="w-64 bg-slate-700 text-white shadow-md px-4 py-6">
        <h2 class="text-lg font-bold mb-4">
          <.link navigate={~p"/albums"}>Albums</.link>
          <.link navigate={~p"/beatles"} class="text-sm font-normal hover:underline">
            View Beatles
          </.link>
        </h2>
        <ul class="space-y-2">
          <li :for={album <- @albums} class="hover:bg-slate-600 hover:text-white rounded">
            <.link navigate={~p"/albums/#{album}"} class="block p-2 w-full h-full">
              {album.name}
            </.link>
          </li>
        </ul>
      ...
      <div class="flex-1 p-6">
        <div :if={@album} class="bg-white shadow-md p-4 rounded-lg">
          ...
          />
          <h3 class="text-xl font-bold mb-2">{@album.name}</h3>
  ...
  def mount(_params, _session, socket) do
    albums = Phoenixdev.Beatles.list_albums()
