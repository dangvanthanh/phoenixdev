defmodule PhoenixdevWeb.ChartsLive do
  ...
  def handle_params(_params, _uri, socket) do
    {:noreply,
     ...
     |> assign_async_schedule([:line_stack, :gauge_multi_title], fn opts ->
       %{
         line_stack: Phoenixdev.Charts.get_line_stack_data(opts[:line_stack][:cumulative]),
         gauge_multi_title: Phoenixdev.Charts.get_gauge_multi_title_data()
       }
     end)
     |> assign_async_schedule(:process_gauge, fn _opts ->
       %{process_gauge: Phoenixdev.Charts.get_process_count()}
     end)}
  end
